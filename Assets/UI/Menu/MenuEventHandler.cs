...
public class MenuEventHandler : MonoBehaviour {
  ...
  private void OnEnable() {
    ...
    _buttonBoxMap = new Dictionary<Button, Box>() {
      { _createButton, _createBox },     { _createBackButton, _createBox },
      { _joinButton, _joinBox },         { _joinBackButton, _joinBox },
      { _findButton, _findBox },         { _findBackButton, _findBox },
      { _settingsButton, _settingsBox }, { _settingsBackButton, _settingsBox }
    };
    ...
    foreach (Button button in _buttonBoxMap.Keys) {
      if (button == null || _buttonBoxMap[button] == null)
        return;
      button.clicked += () => { _buttonBoxMap[button].ToggleInClassList("visible"); };
    }

    _createSubmitButton.clicked +=
        () => { GameManager.Instance.CreateGame(_timerValueMap[_timerValue]); };

    _joinSubmitButton.clicked += () => { GameManager.Instance.JoinGame(); };

    _findSubmitButton.clicked += 
      () => { GameManager.Instance.FindGame(_timerValueMap[_timerValue]); };
